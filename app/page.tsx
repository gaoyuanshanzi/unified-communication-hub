...
export default function DashboardPage() {
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-100/70 overflow-hidden font-sans">
      {/* 상단 통합 헤더 */}
      <header className="flex-shrink-0 bg-white border-b border-slate-200 px-5 py-3 shadow-xs z-10">
        <div className="flex items-center justify-between max-w-full">
          {/* 좌측: 로고 및 타이틀 */}
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center w-9 h-9 rounded-xl bg-blue-600 shadow-xs">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-slate-200 bg-white text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-all shadow-2xs"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-slate-200 bg-white text-xs font-semibold text-slate-600 hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-all shadow-2xs"
            >
              로그아웃
            </button>
          </div>
      ...
      {/* 3열 균등 분할 메일 패널 */}
      <main key={refreshKey} className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-3 gap-3.5 p-3.5">
        {/* 1. 카카오 / 다음 메일 */}
        <NativeMailColumn
          provider="kakao"
